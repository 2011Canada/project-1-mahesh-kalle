...
xhttp.open("GET",'http://localhost:8080/ers5/html/Manager.do',true); //Sends the set values 'request' to this.
xhttp.send();
}
...
function getTicketInfo() {
	...
	xhttp.open("GET",
			'http://localhost:8080/ers5/html/displayAllTickets.do', true);																																			// this.
	xhttp.send();
...
xhttpReq.open("PUT",'http://localhost:8080/ers5/html/approveTicket.do',true); 
...
function denyTicketRequest(row){
	...
	let id = row.ticket_Id;
	//this goes straight to the request helper.
xhttpReq.open("PUT",'http://localhost:8080/ers5/html/denyTicket.do',true); //Sends the set values 'request' to this.
xhttpReq.setRequestHeader("ticketid","x-www-form-urlencoded");
xhttpReq.send(id);
}
